...
export default class Watcher extends _watch {
  addedHook(params: { directory: string; newFile: string }) {
    ...
    const musics = Composer.compose(params.newFile, { isDirectory: false, library: params.directory })

    musics.forEach(async music => {
      const m = await music;

      BrowserWindow.getAllWindows()[0].webContents.send(FrontEndListeners.addToLibrary, m);
    })
